...
int main() {
    ...
    for (;;) {
        ...
        int number_of_fields_read = scanf("%10s", command);
        ...
        if ('l' == command[0] && 'i' == command[1] && 's' == command[2] && 't' == command[3] && '\x00' == command[4]) {
            for (int i = 0; i < student_vector->count; i += 1) {
            ...
            free(student_vector->students+student_vector->count);
            free(command);
        }
        ...
        // add
            ...
            int success = student_vector_add(student_vector, new_student);
            if (success) {
        ...
        // del
            ...
            int success = student_vector_remove(student_vector, i);
            if (success) {
